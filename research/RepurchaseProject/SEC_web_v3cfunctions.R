...
loc.item  <- function(x, # filing 
  ...
  toc_tbl_id <- which(grepl(pattern = ">Part.+I", x = toc_tbl, ignore.case = T) & grepl(pattern = "SIGNATURE|EXHIBIT", x = toc_tbl, ignore.case = T))[1] # locate the table for TOC: will return NA if there are no tables in toc_tbl
